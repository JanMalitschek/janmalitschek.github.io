...
function _DownloadImage(fileName, fileContent) {
 const byteCharacters = atob(UTF8ToString(fileContent));
...
function _LoadFileFromIDB(fileName, onComplete, onFileNotFound) {
 let db;
 const request = window.indexedDB.open("ftDesignStudio", 1);
 const fileNameStr = UTF8ToString(fileName);
 console.log(`Loading ${fileNameStr} from IndexedDB`);
...
function _SaveFileToIDB(fileName, fileContent) {
 ...
 console.log(`Saving ${fileNameStr} to IndexedDB`);
 request.onerror = function(event) {
  console.error(`IndexedDB error: ${event.target.errorCode}`);
 };
 request.onupgradeneeded = function(event) {
